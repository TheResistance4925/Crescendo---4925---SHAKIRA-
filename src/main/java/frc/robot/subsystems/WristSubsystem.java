...
public class WristSubsystem extends SubsystemBase {
  ...
  public WristSubsystem() {
    ...
    m_motor = new CANSparkMax(Constants.Arm.kWristCanId, MotorType.kBrushless);
   ...
   // m_motor.setSoftLimit(SoftLimitDirection.kReverse, (float) Constants.Arm.kSoftLimitReverse);
    ...
    m_encoder.setPositionConversionFactor(Constants.Arm.kWristPositionFactor);
    m_encoder.setVelocityConversionFactor(Constants.Arm.kWristVelocityFactor);
    m_encoder.setPosition(0.0);

    m_controller = m_motor.getPIDController();
    PIDGains.setSparkMaxGains(m_controller, Constants.Arm.kWristPositionGains);

    m_motor.burnFlash();

    m_follower = new CANSparkMax(27, MotorType.kBrushless );
    m_follower.follow(m_motor);
    m_follower.setInverted(true);

    m_follower.burnFlash();

    m_setpoint = Constants.Arm.kWristHomePosition;
   ...
   */
  public void setWristTargetPosition(double _setpoint) {
  ...
  private void updateMotionProfile() {
    ...
    m_profile = new TrapezoidProfile(Constants.Arm.kWristMotionConstraint);
    m_timer.reset();
  }
  ...
  public void runAutomatic() {
    ...
    m_feedforward =
        Constants.Arm.kWristFeedforward.calculate(
            m_encoder.getPosition() + Constants.Arm.kWristZeroCosineOffset, m_targetState.velocity);
